...
author: "Augustin Antier - Louis Belle - Erwan Gouhier - Riwal Le Moan Delalande -
  Louis Prusiewicz "
date: "2025-11-14"
output:
  pdf_document:
...
classification ascendante hiérarchique sera mise en place.
...
entre deux variables. Dans notre cas nous avons choisi comme définition
...
**Deux variables éloignées** sont deux variables orthogonales qui
...
saumon pour ses aspects bénéfique pour la santé (oméga 3). C'est un
groupe ou les personnes qui consomment le saumon sont principalement des
...
pas décorrélée de la réalité physique des produits. On va donc projeter
...
rassure quand à la consistance de nos données. Nos données nous semblent
donc relativement fiables puisque les préférences des consommateurs sont
...
## Exemple de loi bidimensionnelles :
...
## Comment générer un loi de mélange gaussienne
...
groupe. On génère notre vecteur avec le code suivant :
...
On commence par l'étape **d'expectation** (E) de l'algorithme EM. Cette
...
pics, qui vont correspondre a nos moyennes, on estime également un
...
notre cas nous avons choisi la **log vraisemblance **. Notre algorithme
s'arrête lorsque la log vraisemblance  de l'étape précédente et celle de
...
On créer enfin la fonction du calcul de la log-vraisemblance .
...
coefficients directeurs  pour la classification correspond à une
...
leurs features n'est pas déconnant.
...
On représente graphiquement les coefficients  directeurs sur les trois
...
(appelés **documents** par la suite), afin de produire un ensemble de
sommaire de **termes** (mots) représentant l'ensemble  des **thèmes
...
La première étape consiste à créer les topics en leur attribuant  des
mots.
...
documents. Une fois le Topics selectionné on tire un mot dans la
distribution de mot, c'est ce mot tiré qui sera notre premier mot. Ce
processus  est répété pour chaque mot du document.
...
faire le Topic modeling. Pour celà nous allons faire un exemple simple

**Doc1** : Une *banque* me *prête * de l'*argent* pour une *dette* 
...
Pour affiner les Topics, on va sortir un mot d'un topic, et le réaffecter 
